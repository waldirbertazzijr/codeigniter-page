 ...
 * @copyright	Copyright (c) 2010 - 2011, Waldir Bertazzi Junior
 ...
 * @category	Pages
 * @author		Waldir Bertazzi Junior
 * @link		http://waldir.org/orion/doc/
 */
class Page {
	...
	function __construct($args = null){		
        ...
        // essential helpers
        $this->CI->load->helper(array('orion', 'min_url'));
        
        // essential libraries
        $this->CI->load->library(array('weblibrary', 'session'));
	...
	function js($filename, $from_lib=FROM_USER){
		...
		if(is_array($filename)){
			...
			if($file = has_extension($filename)){
				array_pop($file);
				if(!$from_lib)
	...
	**/
	...
	function messages(){
		...
		if ($this->CI->session->flashdata('error_message')):
			...
			$return .= '</span></div>';
		// A sexy success message
        elseif ($this->CI->session->flashdata('success_message')):
	...
	function generate($title=""){
		...
		// pin the base url for usage inside javascript scripts, very useful for javascript frameworks
		$this->code("// Util variables\n\t\tvar site_url = '".site_url()."';\n\t\tvar base_url = '".base_url()."';");
	
		$head_info = "<!--\n  ▽ Orion Framework\n\n'All you need is love, love,\nLove is all you need'\n\t\t- John Lennon -->\n\n";
		
		// template header
		$return .= $this->CI->load->view("../templates/{$this->template}/header", array('head_info'=>$head_info, 'title'=> $title, 'metas' => $this->_compile_metas(), 'css'=>$this->_compile_css(), 'js'=>$this->_compile_js(), 'custom'=>$this->custom_tags), $this->ret);
		...
		// template footer
		$return .= $this->CI->load->view("../templates/{$this->template}/footer", array('footer_js'=>$this->_compile_weblibraries_js()), $this->ret);
	...
	private function _compile_css(){
		...
		foreach($this->lib_css as $css_file){
			$compiled_css .= "\t<link rel=\"stylesheet\" href=\"".assets_url('weblibraries')."{$css_file}.css\" />\n";
		}
		foreach($this->css as $css_file){
			$compiled_css .= "\t<link rel=\"stylesheet\" href=\"".assets_url('css')."{$css_file}.css\" />\n";
		}
	...
	private function _compile_js(){
		...
		foreach($this->lib_js as $js_file){
			$compiled_js .= "\t<script type=\"text/javascript\" src=\"".assets_url('weblibraries')."{$js_file}.js\"></script>\n";
		}
		foreach($this->js as $js_file){
			$compiled_js .= "\t<script type=\"text/javascript\" src=\"".assets_url('javascript')."{$js_file}.js\"></script>\n";
		}
	 ...
	 * @return	string
	 */
	function _compile_weblibraries_js(){
